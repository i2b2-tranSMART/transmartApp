		...
		for (var i = 0; i < cm.getColumnCount(); i++) {
			if (includeHidden || !cm.isHidden(i)) {
				...
				headerXml += '<ss:Cell ss:StyleID="headercell">' +
					'<ss:Data ss:Type="String">' + cm.getColumnTooltip(i) + '</ss:Data>' +
...
//	  Generate worksheet header details.
	...
	getCsv: function () {
		...
		for (i = 0; i < columnCount; i++) {
			...
			header.push(columnModel.getColumnTooltip(i));

			switch (this.store.recordType.prototype.fields.get(columnModel.getDataIndex(i)).type) {
				case 'int':
				case 'float':
					cellTypes.push('Number');
					break;
				case 'date':
					cellTypes.push('DateTime');
					break;
				default: // includes 'bool', 'boolean'
		...
		if (this.getSelectionModel().getSelections().length > 0) {
			...
			contentRows = this.store.data.items;
		}

		var csv = header.join(',') + '\n';

		var length = contentRows.length;
		for (i = 0; i < length; i++) {
			var csvRow = [];
			var rowData = contentRows[i].data;
			var visibleIndex = 0;
			for (var j = 0; j < columnCount; j++) {
				if (columnModel.isHidden(j)) {
					continue;
				}
				var value = rowData[columnModel.getDataIndex(j)];
				var columnType = (value == null || value.toString() === '') ? 'String' : cellTypes[visibleIndex];
				if (columnType === 'DateTime') {
					csvRow.push(value.format('Y-m-d'));
				}
				else {
					csvRow.push(value);
				}
