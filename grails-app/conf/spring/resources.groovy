...
beans = {
	...
	if (SpringSecurityUtils.securityConfig.ldap.active) {
		...
		if (grailsApplication.config.grails.plugin.springsecurity.providerNames.contains('kerberosServiceAuthenticationProvider')) {
			...
			// only active this if both LDAP & Kerberos providers are enabled to avoid ClassCastException (due to https://github.com/grails-plugins/grails-spring-security-kerberos/issues/3 )
		}
	}
