...
class ChartController {
	...
	def conceptDistribution = {
		...
		accessLogService.report 'DatasetExplorer-Set Value Concept Histogram', 'Concept:' + params.concept_key
	...
	def conceptDistributionForSubset = {
		...
		accessLogService.report 'DatasetExplorer-Set Value Concept Histogram for subset', 'Concept:' + params.concept_key
	...
	def conceptDistributionWithValues = {
		accessLogService.report 'DatasetExplorer-Concept Distribution With Values', 'Concept:' + params.concept_key
	...
	def analysis = {
		...
		accessLogService.report 'DatasetExplorer-Analysis by Concept',
				'RID1:' + params.result_instance_id1 + " RID2:" + params.result_instance_id2 + " Concept:" + params.concept_key
	...
	def analysisGrid = {
		...
		if (table == null) {
			...
			highDimConcepts.each {
				if (s1) {
					highDimensionQueryService.addHighDimConceptDataToTable(table, it, result_instance_id1)
				}
				if (s2) {
					highDimensionQueryService.addHighDimConceptDataToTable(table, it, result_instance_id2)
				}
			}
		}
		PrintWriter pw = new PrintWriter(response.getOutputStream());
		...
		if (concept_key && !concept_key.isEmpty()) {
			...
			if (omics_params) {  // empty maps are coerced to false by groovy
				...
				if (s1) {
					highDimensionQueryService.addHighDimConceptDataToTable(table, omics_params, result_instance_id1)
				}
				if (s2) {
					highDimensionQueryService.addHighDimConceptDataToTable(table, omics_params, result_instance_id2)
				}
			}
			else {
