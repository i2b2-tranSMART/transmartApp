...
class RWGVisualizationDAOService {
	...
	/**
	 * Retrieve a list of gene names for the give probes (identified by probe name)
	 *
	 ...
	 *
	 * @return a map of containing one map for each probe - contains 4 items - a) the short display
	 *         (e.g. first gene with a plus indicator if more than one) and b) long display containing
	 *         the full list of gene names and c) display for select box d) gene Id
	 */
	def getGenesForProbes(probes, boolean indexByProbeName = false, analysisId = null, boolean showSigResultsOnly = false) {
 ...
 * */
	...
	/**
	 * @return the last data load time for faceted search
	 */
