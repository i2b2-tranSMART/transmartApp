...
class GeneExpressionDataService {
	...
	boolean getData(List studyList, File studyDir, String fileName, String jobName, String resultInstanceId,
		...
		try {
			...
			studyList.each { study ->
				...
				def writeDataStatusMap = writeData(resultInstanceId, sqlQuery, sampleQuery, studyDir, filename, jobName,
						includePathwayInfo, splitAttributeColumn, gplIds)
	...
	String createStudySampleAssayQuery(String study, String resultInstanceId, List gplIds, String timepoint,
	 ...
	 */
	String createMRNAHeatmapPathwayQuery(String study, String resultInstanceId, List gplIds, String pathwayName,
	   ...
	   """)
		...
		sSelect.append(sTables)

		sSelect
	}

	String createDownloadCELFilesQuery(String resultInstanceId, studyList, String subjectIds, String timepoint,
	                                   String sampleTypes, String tissueTypes) {
	 ...
	 */
	String getAssayIds(String resultInstanceId, String sampleTypes, String timepoint, String tissueTypes) {
		...
		convertList(assayIdsArray, false, 1000)
	}
	...
	def writeData(String resultInstanceId, String sqlQuery, String sampleQuery, File studyDir, String fileName,
	              String jobName, includePathwayInfo, splitAttributeColumn, gplIds) {
		def filePath = null
		...
		try {
			while (rs.next()) {
				...
				if (splitAttributeColumn) {
					sttSampleStr = new StringBuilder().append(sampleType ?: '').append(valueDelimiter)
							.append(timepoint ?: '').append(valueDelimiter)
							.append(tissueType ?: '').append(valueDelimiter)
							.append(GPL_ID ?: '')

				}
				else {
					sttSampleStr = new StringBuilder().append(sampleType ?: '')
							.append(timepoint ? '_' + timepoint : '')
							.append(tissueType ? '_' + tissueType : '')
							.append(GPL_ID ? '_' + GPL_ID : '')
			...
			while (rs.next()) {
				...
				if (gplIds.size() > 1) { // when there are more than one platforms
					...
					if (gplID.equals(platformToUse)) { // compared with the hard-coded value GPL96
						...
						if (probeExistsInGPL96) {
							continue
						}
// don't write the record for probe that already exists in GPL96
					}
				}
	...
	private String derivePathwayName(String pathway_name) {
		if (!pathway_name || pathway_name == "null") {
			pathway_name = null
		}
	...
	private List getCELFiles(String studyName, String sampleCd) {
		...
		sql.rows(sqlQuery, [studyName, sampleCd + '%'])
	}

	def downloadCELFiles(String resultInstanceId, studyList, File studyDir, String jobName, String pathway,
	                     String timepoint, String sampleTypes, String tissueTypes) {
		Sql sql = null
		...
		try {
			...
			sql.eachRow(sqlQuery, { row ->
				...
				sample = new StringBuilder().append(row.SAMPLE_TYPE?.toString() ?: '')
						.append(row.TIMEPOINT ? '_' + row.TIMEPOINT : '')
						.append(row.TISSUE_TYPE ? '_' + row.TISSUE_TYPE : '')
	...
	void getGCTData(List studyList, File studyDir, String fileName, String jobName,
	...
	void getCLSData(List studyList, File studyDir, String fileName, String jobName,
	...
	private getResultInstanceIdsAsStr(Map resultInstanceIdMap) {
	...
	boolean isMultipleResultInstanceIds(str) {
...
// don't write the record for probe that already exists in GPL96
	...
	def getGplTitle(gplId) {
		...
		new Sql(dataSource).eachRow(commandString, [gplId], { row ->
			gplTitle = row[0]
		})
		return gplTitle
	}
