...
class GeneSignatureService {
		...
		try {
			...
			while (br.ready() && i < 3) {
				...
				if (items.size() != colCount) {
					throw new FileSchemaException("Invalid number of columns, please check file:'" + origFile + "' settings and/or correct usage of tab delimiter")
				}
					...
					case METRIC_CODE_TRINARY:
						...
						try {
							...
							if (triFoldChg != -1 && triFoldChg != 0 && triFoldChg != 1) {
								throw new FileSchemaException("ERROR: Fold-change value (" + triFoldChg + ") in file:'" + origFile + "' did not match one of the trinary indicators (i.e. -1,0,1)!")
							}
						}
					...
					case METRIC_CODE_ACTUAL:
						...
						try {
							...
							if (actFoldChg == -1 || actFoldChg == 0 || actFoldChg == 1) {
								throw new FileSchemaException("Fold-change value (" + foldChgTest + ")  in file:'" + origFile + "' appears to be trinary instead of an actual fold change!")
							}
						}
						catch (NumberFormatException e) {
		...
		try {
			...
			while (br.ready()) {
				...
				if (fileSchemaName.toUpperCase() =~ /GENE /) {
					...
					marker = lookupProbesetBioAssociations_probeIds(geneSymbol, gs.techPlatform.accession)
					// fixes -- 2014_10_31
	...
	def addGenSigItems(GeneSignature gs, List<String> geneSymbols, List<String> probes, List<Double> valueMetrics) {
		...
		def metricItems
		if (valueMetrics) {
			metricItems = valueMetrics.toArray()
		}
	 ...
	 */
	List<GeneSignature> listPermissionedGeneSignatures(Long userId, boolean bAdmin) {
	...
	def getPermissionedCountMap(Long userId, boolean bAdmin) {
		...
		*/
		...
		nativeSQL.append("from SEARCHAPP.SEARCH_GENE_SIGNATURE_ITEM gsi join SEARCHAPP.SEARCH_GENE_SIGNATURE gs on gsi.search_gene_signature_id=gs.search_gene_signature_id ")
