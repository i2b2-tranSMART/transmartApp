...
class I2b2HelperService {
     ...
     */
    double[] getPatientDemographicValueDataForSubset(String col, String result_instance_id) {
     ...
     */
    String keyToPath(String concept_key) {
     ...
     */
    String getParentConceptKey(String concept_key) {
     ...
     */
    String getConceptKeyForAnalysis(String concept_key_in) {
     ...
     */
    int getLevelFromKey(String key) {
     ...
     */
    boolean isHighDimensionalConceptKey(String concept_key) {
     ...
     */
    boolean isLeafConceptKey(String concept_key) {
     ...
     */
    boolean isLeafConceptKey(AcrossTrialsOntologyTerm conceptItem) {
        // profuse appoligies to future programmers reading this code; it is clearly a mess
        // and this is a patch on top of a mess; the correct solution is to rewrite all this code
        // to use the API and to rewrite the underlying object class to use the API.
        //log.trace "----------------- isLeafConceptKey - AcrossTrialsOntologyTerm case"
        EnumSet probeSet = conceptItem.visualAttributes
        if (!probeSet) return false
        return probeSet.any{ it == OntologyTerm.VisualAttributes.LEAF }
    }
     ...
     */
    int getPatientSetIntersectionSize(String result_instance_id1, String result_instance_id2) {
    ...
    SortedMap<String, Map<String, Integer>> getConceptDistributionDataForConceptByTrial(String concept_key, String result_instance_id) throws SQLException {
     ...
     */
    Map<String, Integer> getConceptDistributionDataForConceptOld1(String concept_key, String result_instance_id) throws SQLException {
        checkQueryResultAccess result_instance_id
     ...
     */
    Map<String, Integer> getConceptDistributionDataForConceptOld2(String concept_key, String result_instance_id) throws SQLException {
        log.debug "----------------- start getConceptDistributionDataForConcept"
        String fullname = concept_key.substring(concept_key.indexOf("\\", 2), concept_key.length())
        Map<String, Integer> results = [:]
     ...
     */
    List<String> getChildValueConceptsFromParentKey(String concept_key) {
    ...
    Map<String, Integer> getAllObservationCountsForXTrialsConceptNodeWithTrial(String trial, String concept_key, String result_instance_id)  throws SQLException {
        checkQueryResultAccess result_instance_id

        log.debug "------ getAllObservationCountsForXTrialsConceptNodeWithTrial"

        def node = conceptsResourceService.getByKey(concept_key)
        List<OntologyTerm> childNodes = node.children
    ...
    List<AcrossTrialsOntologyTerm> getAllXTrialsLeafNodes(AcrossTrialsOntologyTerm top){
     ...
     */
    ExportTableNew addConceptDataToTable(ExportTableNew tablein, String concept_key, String result_instance_id) {
        checkQueryResultAccess result_instance_id
     ...
     * */
    Map<String, Integer> getPatientDemographicDataForSubset(String col, String result_instance_id) {
     ...
     */
    List<String> getConceptKeysInSubset(String resultInstanceId) {
     ...
     */
    List<String> getConceptKeysInSubsets(String result_instance_id1, String result_instance_id2) {
    ...
    Set<String> lookupChildConcepts(String parentConcept, String result_instance_id1, String result_instance_id2) {
        checkQueryResultAccess result_instance_id1, result_instance_id2
    ...
    Set<String> lookupChildConcepts(String parentConcept, String result_instance_id) {
     ...
     */
    List<String> getConceptsAsListFromSample(List SampleIDList) {
     ...
     */
    String getConceptsFromKey(String key) throws SQLException {
        //String slash="\\"
     ...
     */
    List<String> getConceptsFromKeyAsList(String key) throws SQLException {
        //String slash="\\"
        //logMessage("Here is slash: "+slash)
    ...
    def getSampleHeatMapData(String pathwayName,
        ...
        Map<Long, SampleInfo> sampleInfoMap = [:]
        for (SampleInfo sampleInfo : sampleInfoList) {
        ...
        try {
            ...
            sql.eachRow(sqlStr) { row ->
                ...
                if (curGeneSymbol == null || curGeneSymbol != geneSymbol) {
                    // reached next gene
    ...
    def getSurvivalAnalysisData(List<String> concepts1, List<String> concepts2, List<String> subjects1, List<String> subjects2,
        ...
        survivalDataList_1.eachWithIndex() { obj, i ->
            ...
            if (obj.isEvent != null && obj.isEvent.booleanValue()) {
                dataBuf.append("1\n")
            } else {
                dataBuf.append("0\n")
            }
    ...
    List<SurvivalData> getSurvivalDataForSubset(List<String> subjectStrList, List<String> conceptStrList) {
     ...
     */
    void defineConceptsForSurvivalAnalysis(List<Concept> concepts, SurvivalConcepts survivalConcepts) {
        if (survivalConcepts == null) {
            return
        }
        List<Concept> conceptNumericalList = []
    ...
    Map<String, SurvivalData> getSurvivalDataForSurvivalTime(Concept conceptSurvivalTime, List<String> subjectStrList) {
    ...
    void getSNPDataByProbeByChrom(List<SnpDataset> datasetList, Map<String, List<SnpDataByProbe>> snpDataByChromMap,
        ...
        sql.eachRow(sqlStr, [trialName]) { row ->
            ...
            Clob clob = (Clob) row.data_by_probe
    ...
    void getGwasDataByPatient(List<String> subjectIdList1, List<String> subjectIdList2, String chroms, GwasFiles gwasFiles) {
        if (gwasFiles == null) {
    ...
    def reportGwas(String userName, GwasFiles gwasFiles, String querySummary1, String querySummary2) {
        ...
        if (assocFile == null || !assocFile.isFile()) {
        ...
        if (datasetCountList != null) {
            if (datasetCountList && datasetCountList.get(0) != null) {
    ...
    Map<String, String[]> getChromEndProbeLineMap() {
        ...
        Map<String, String[]> chromEndProbeLineMap = [:]
    ...
    List<String> getSNPPlatformList(String subjectIds) {
        if (!subjectIds?.trim()) {
            return null
        }
    ...
    String getSNPPlatform(String subjectIds1, String subjectIds2) {
        ...
        if (platformList2 != null) {
            if (platformList2.size() == 1) {
                ...
                if (platform == null) {
                    platform = platform2
                } else if (platform != null && platform2 != platform) {
                    throw new Exception("More than 1 SNP platforms exist in datasets.")
                }
            } else {
                throw new Exception("More than 1 SNP platforms exist in datasets.")
    ...
    void fillCensoringToSurvivalData(Concept conceptCensoring, Map<String, SurvivalData> dataMap, List<String> subjectStrList) {
        if (conceptCensoring == null || conceptCensoring.getBaseCode() == null || !dataMap) {
            return
        }
        Sql sql = new Sql(dataSource)
        String subjectIdListInStr = DBHelper.listToInString(subjectStrList)
    ...
    void fillEventToSurvivalData(Concept conceptEvent, Map<String, SurvivalData> dataMap, List<String> subjectStrList) {
        if (conceptEvent == null || conceptEvent.getBaseCode() == null || !dataMap) {
            return
        }
        Sql sql = new Sql(dataSource)
        String subjectIdListInStr = DBHelper.listToInString(subjectStrList)
    ...
    String getTrialName(String ids) {
     ...
     * @return
     */
    String getTrialNameBySampleID(String ids) {
    ...
    String getAssayIds(String ids, String sampleTypes, String timepoint) {
    ...
    String createMRNAHeatmapQuery(String ids, String concepts, String pathwayName, String timepoint) {
    ...
    String getSubjectIds1(String ids1, String ids2, String prefix1, String prefix2) {
    ...
    String getSubjectIds(String ids1, String ids2, String prefix1, String prefix2, String separator) {
        String[] idArray1
        String[] idArray2
    ...
    String[] getSubjectIdArray(String ids1, String ids2, String prefix1, String prefix2) {
        Integer[] idArray1
        Integer[] idArray2
        ...
        if (ids1) {
            ...
            for (int i = 0; i < idStringArray.length; i++) {
                idArray1[i] = Integer.parseInt(idStringArray[i])
            }
            idArray1.sort()
        }
        if (ids2) {
            ...
            for (int i = 0; i < idStringArray.length; i++) {
                idArray2[i] = Integer.parseInt(idStringArray[i])
            }
            idArray2.sort()
        }
        String[] ids
    ...
    String createRBMHeatmapQuery(String prefix, String ids, String concepts, String pathwayName, String timepoint, String rbmPanels) {
    ...
    String createProteinHeatmapQuery(String prefix, String pathwayName,
    ...
    String createProteinHeatmapQuery(String pathwayName, String ids1, String ids2, String concepts1,
    ...
    String createRBMHeatmapQuery(String pathwayName, String ids1, String ids2, String concepts1, String concepts2,
     ...
     */
    String createMRNAHeatmapQuery(String pathwayName, String ids1, String ids2, String concepts1,
                                  String concepts2, String timepoint1, String timepoint2) {
        return createMRNAHeatmapBaseQuery(pathwayName, ids1, ids2, concepts1, concepts2, timepoint1, timepoint2, "LOG2")
    }

    String createMRNAHeatmapSelect(String ids1, String ids2, String prefix1, String prefix2) {
     ...
     */
    String listHeatmapColumns(String biomarker, String ids1, String ids2, String prefix1, String prefix2) {
     ...
     */
    String createMRNAHeatmapBaseQuery(String pathwayName, String ids1, String ids2, String concepts1,
                                      String concepts2, String timepoint1, String timepoint2, String sample1,
                                      String sample2, String intensityType) {
        createMRNAHeatmapBaseQuery(pathwayName, ids1, ids2, concepts1, concepts2, timepoint1, timepoint2,
            sample1, sample2, intensityType, false)
    }

    String createMRNAHeatmapBaseQuery(String pathwayName, String ids1, String ids2, String concepts1,
                                      String concepts2, String timepoint1, String timepoint2, String sample1,
                                      String sample2, String intensityType, boolean count) {
    ...
    String createMRNAHeatmapPathwayQuery(String prefix, String ids, String concepts, String pathwayName,
     ...
     * Gets the children value type concepts of a parent key
     */
    List<String> getChildPathsFromParentKey(String concept_key) {
        String prefix = concept_key.substring(0, concept_key.indexOf("\\", 2))
        //get the prefix to put on to the fullname to make a key
        String fullname = concept_key.substring(concept_key.indexOf("\\", 2), concept_key.length())
    ...
    String getTrialNameByConceptKey(String key) {
        return conceptsResourceService.getByKey(key).getStudy().getId()
    }

    List<String> trialsForResultSet (String result_instance_id) {
        checkQueryResultAccess result_instance_id
