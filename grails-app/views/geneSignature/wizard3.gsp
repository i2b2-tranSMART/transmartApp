...
<!--  show message -->
	<g:if test="${flash.message}">
		...
		<g:hasErrors bean="${wizard.geneSigInst}">
			<div class="errors"><g:renderErrors bean="${wizard.geneSigInst}" as="list"/></div>
		</g:hasErrors>
		<br>
	</g:if>
...
<!-- instructions -->
	...
	<g:uploadForm name='geneSignatureFrm'>
		...
		<table class="detail">
			<tr class="prop">
				...
				<td class="value">
					<g:textField name="analystName" value="${gs.analystName}" size="100%" maxlength="100"/>
				</td>
			</tr>
			<tr class="prop">
				<td class="name">Normalization Method</td>
				<td class="value">
					<g:select name='normMethodConceptCode.id' from="${wizard.normMethods}"
					          ...
					          onchange="toggleOtherDiv(this, 'norm_method_other_div');"/>
				<!--  toggle other div accordingly -->
					...
					<label>Please provide 'other' detail<g:requiredIndicator/>:</label><br><g:textField
						name="normMethodOther" value="${gs.normMethodOther}" size="100%" maxlength="255"/>
				</div>
				</td>
			</tr>
			<tr class="prop">
					...
					<table>
						<tr>
							...
							<td style="border: none;">
								<g:select name='analyticCatConceptCode.id' from="${wizard.analyticTypes}"
								          value="${existingValues.'analyticCatConceptCode.id'}"
								          noSelection="['null': 'select category']"
								          optionValue="${{
									          (it?.bioConceptCode == 'OTHER') ? it?.codeName : it?.codeName + ' (' + it?.codeDescription + ')'
								          }}"
								          optionKey='id' onchange="toggleOtherDiv(this, 'analysis_cat_other_div');"/>
							<!--  toggle other div accordingly -->
								...
								<label>Please provide 'other' detail<g:requiredIndicator/>:</label><br><g:textField
									name="analyticCatOther" value="${gs.analyticCatOther}" size="100%" maxlength="255"/>
							</div>
							</td>
						</tr>
						<tr>
							...
							<td style="border: none;">
								<g:select name='analysisMethodConceptCode.id' from="${wizard.analysisMethods}"
								          value="${existingValues.'analysisMethodConceptCode.id'}"
								          noSelection="['null': 'select analysis method']"
								          optionValue='codeName' optionKey='id'
								          onchange="toggleOtherDiv(this, 'analysis_method_other_div');"/>
							<!--  toggle other div accordingly -->
								...
								<label>Please provide 'other' detail<g:requiredIndicator/>:</label><br><g:textField
									name="analysisMethodOther" value="${gs.analysisMethodOther}" size="100%" maxlength="255"/>
							</div>
							</td>
						</tr>
					</table>
			...
			<tr class="prop">
				...
				<td class="value">
					<g:select name='pValueCutoffConceptCode.id' from="${wizard.pValCutoffs}"
			...
			<tr>
				 <td colspan="2" style="font-weight: bold; font-size: 12px;">
				   ...
				   <a style="font-style:italic;" href="${resource(dir: 'images', file: 'gene_sig_samples.txt')}" target="_blank">
					<img alt="examples" src="${resource(dir: 'images', file: 'text.png')}"/>&nbsp;See Samples</a>
			</td>
		</tr>
		</g:if>
		<g:else>
			<p style="font-weight: bold;">File Upload Information (tab delimited text only, no .xls Excel files):&nbsp;&nbsp;
				<a style="font-style:italic;" href="${resource(dir: 'images', file: 'gene_sig_samples.txt')}"
				   target="_blank"><img alt="examples" src="${resource(dir: 'images', file: 'text.png')}"/>&nbsp;See Samples
				</a></p>
			<table class="detail" style="width: 100%">
