...
<div class="body">
	...
	<g:form frm='geneSignatureItemFrm'>
		...
		<table class="detail">
			...
			<tr>
				<th>#</th>
				<g:if test="${gs?.fileSchemaId != 3}"><th>Gene Symbol</th></g:if>
				<g:if test="${gs?.fileSchemaId == 3}"><th>Probeset ID</th></g:if>
			...
			<g:each var="item" in="${gs.geneSigItems}" status="i">
				<tr>
					...
					<g:if test="${gs?.fileSchemaId != 3}"><td>${item.bioMarker?.name}</td></g:if>
					<g:if test="${gs?.fileSchemaId == 3}"><td>${item.probeset}</td></g:if>
		...
		<br>
		<table class="detail">
			<g:tableHeaderToggle label="Expand to Add Items" divPrefix="${gs.id}_new_items"
			                     colSpan="${(gs.foldChgMetricConceptCode.bioConceptCode != 'NOT_USED') ? '4' : '3'}"/>
			<tbody id="${gs.id}_new_items_detail" style="display: none;">
			<tr id="new_header">
				<th style="text-align: center;">#</th>
				<g:if test="${gs?.fileSchemaId != 3}"><th style="text-align: center;">Gene Symbol</th></g:if>
				<g:if test="${gs?.fileSchemaId == 3}"><th style="text-align: center;">Probeset ID</th></g:if>
				<g:if test="${gs.foldChgMetricConceptCode?.bioConceptCode != 'NOT_USED'}"><th
						style="text-align: center;">Fold-Change Metric</th></g:if>
			...
			<g:while test="${n < 10}">
				...
				<!-- check if coming from an error -->
					<g:if test="${errorFlag}">
						<g:if test="${gs?.fileSchemaId != 3}">
							<td style="text-align: center;"><g:textField name="biomarker_${n}"
							                                             value="${params['biomarker_' + n]}"
							                                             maxlength="25"/></td>
						</g:if>
						<g:if test="${gs?.fileSchemaId == 3}">
							<td style="text-align: center;"><g:textField name="probeset_${n}"
							                                             value="${params['probeset_' + n]}"
							                                             maxlength="25"/></td>
						</g:if>
						<g:if test="${gs.foldChgMetricConceptCode?.bioConceptCode != 'NOT_USED'}"><td><g:textField
								name="foldChgMetric_${n}" value="${params['foldChgMetric_' + n]}"
					...
					<g:else>
						...
						<g:if test="${gs.foldChgMetricConceptCode?.bioConceptCode != 'NOT_USED'}"><td><g:textField
								name="foldChgMetric_${n}" maxlength="20"/></td></g:if>
					</g:else>
