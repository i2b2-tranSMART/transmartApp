...
<div id="jubresult" style="display: block;">
	<g:if test="${searchresult?.literatureCount() > 0}">
			...
			<tr>
				<td>
					<table>
						...
						<tr><td style="padding:2px 0 0 20px;">Alterations (${searchresult?.litJubOncAltCount})</td></tr>
						<tr><td style="padding:2px 0 0 20px;">Inhibitors (${searchresult?.litJubOncInhCount})</td></tr>
						<tr><td style="padding:2px 0 0 20px;">Interactions (${searchresult?.litJubOncIntCount})</td></tr>
					</table>
				</td>
				<td>
					<table>
						<tr><td><b>Jubilant Asthma (${searchresult?.litJubAsthmaCount()})</b></td></tr>
						<tr><td style="padding:2px 0 0 20px;">Alterations (${searchresult?.litJubAsthmaAltCount})</td></tr>
						<tr><td style="padding:2px 0 0 20px;">Interactions (${searchresult?.litJubAsthmaIntCount})</td></tr>
						<tr><td style="padding:2px 0 0 20px;">Protein Effects (${searchresult?.litJubAsthmaPECount})</td></tr>
					</table>
				</td>
				<td width="50%">&nbsp;</td>
			</tr>
		...
		<div class="paginateButtons" style="width: 100%;">
			...
			<select class="jubselect" name="resultType" id="resultType" style="width:240px;"
				...
				<g:if test="${searchresult?.litJubOncAltCount > 0}">
					<option value="JUBILANT_ONCOLOGY_ALTERATION" ${!searchresult.resultType || searchresult.resultType == 'JUBILANT_ONCOLOGY_ALTERATION' ? "selected" : ""}>Jubilant Oncology Alterations</option>
				</g:if>
		...
		<table width="100%">
												...
												<g:if test="${jubData.reference.referenceId?.startsWith('NCT')}">
													<a style="border: none;" target="_blank" title="Clinical Trials"
													   href="http://www.clinicaltrials.gov/ct2/show/${jubData.reference.referenceId}">
														<img alt="CT" src="${resource(dir: 'images', file: 'clinicaltrials.png')}"/>&nbsp;Reference
													</a>
												</g:if>
												<g:elseif test="${jubData.reference.referenceId?.startsWith('ISRCTN')}">
													<a style="border: none;"
													   ...
													   target="_blank" title="Controlled Trials">
														<img alt="ISRCTN" src="${resource(dir: 'images', file: 'controlledtrials.png')}"/>&nbsp;Reference
													</a>
												</g:elseif>
												<g:else>
													<a style="border: none;"
													   ...
													   target="_blank" title="PubMed ID">
														<img alt="PubMed" src="${resource(dir: 'images', file: 'ncbi.png')}"/>&nbsp;Reference
													</a>
												</g:else>
											</g:if>
										</g:else>
									...
									<td colspan="2">
										<span style="white-space:nowrap">
											...
											<g:if test="${jubData.reference.geneId?.length() > 0}">
												&nbsp;<b class="filter-item filter-item-gene">Gene:</b>
												${createFilterDetailsLink(altId: jubData.reference.geneId, label: jubData.reference.component, type: "gene")}&nbsp;&nbsp;|
											</g:if>
			...
			<g:else>
				<tr>
					<td style="padding: 10px 0 10px 0;font-weight:bold;text-align:center;">No Results Found</td>
				</tr>
			</g:else>
		</table>
	</g:if>
