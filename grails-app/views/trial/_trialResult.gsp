		...
		<table width="100%" class="trborderbottom">
			...
			<tr>
				<td width="100%" class="bottom">
					<table width="100%">
						<tr>
							<td style="padding: 5px 0px 5px 5px; margin-top: 5px;">
								<div id="TrialDet_${trialresult.trialId}_anchor">
								<a onclick="if (divIsEmpty('${trialresult.trial.trialNumber}_detail')) {
									var ldiv = '${trialresult.trial.trialNumber}_detail_loading';
									${remoteFunction(action:'getTrialAnalysis',controller:'trial', id:trialresult.trialId, before:'toggleVisible(ldiv);', onComplete:'toggleVisible(ldiv);', update:trialresult.trial.trialNumber+'_detail')};
								};
								...
								<a onclick="showDialog('TrialDet_${trialresult.trialId}', {
									title: '${trialresult.trial.trialNumber}',
									url: '${createLink(action:'expDetail',id:trialresult.trialId)}'
								});"
								   onmouseover="delayedTask.delay(2000, showDialog, this, ['TrialDet_${trialresult.trialId}', {
									   title: '${trialresult.trial.trialNumber}',
									   url: '${createLink(action:'expDetail',id:trialresult.trialId)}'
								   }]);"
								...
								<br>
							&nbsp;&nbsp;&nbsp;
								<g:set var="pt" value="${trialresult.protocol}"/>
								<g:if test="${pt != null}">
							...
							&nbsp;&nbsp;&nbsp;|
								...
								<g:each in="${trialresult.files}" status="fileCount" var="file">
									<g:if test="${fileCount > 0}">,</g:if>
									<g:else><g:set var="hasFiles" value="${true}"/></g:else>
