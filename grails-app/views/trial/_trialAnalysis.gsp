...
<g:if test="${trialresult.bioMarkerCt > 1}">
	...
	<table style="background-color: #ffffff;" width="100%">
		<g:tableHeaderToggle
				label="(${trialresult.analysisCount - trialresult.inSignificantAnalCount}) Significant TEA Analyses"
				divPrefix="significant_block" status="open"/>
		...
		<g:each in="${trialresult.analysisResultList}" status="i" var="analysisResult">
			<g:if test="${analysisResult.bSignificantTEA}">
				...
				<g:render template='/trial/teaAnalysisSummary'
				          model="[analysisResult: analysisResult, counter: counter, showTrial: showTrial]"/>
			</g:if>
		</g:each>
		</tbody>
	</table>
	...
	<g:if test="${trialresult.inSignificantAnalCount > 0}">
		<table style="background-color: #ffffff;" width="100%">
			<g:tableHeaderToggle label="(${trialresult.insigAnalResultList.size()}) Insignificant TEA Analyses"
			                     divPrefix="insignificant_block"/>
