...
<g:set var="anadivid" value="al_sig_${ar.analysisId}"/>
<g:set var="bmDivId" value="${ar.analysisId + '_' + ar.experimentId}"/>
						...
						<tr>
							<td width="70%">
								...
								<!--  show trial -->
									...
									<a onclick="javascript:showDialog('TrialDetail_${anadivid}', {
										title: '${ar.experimentAccession}',
										url: '${createLink(action:'expDetail',id:ar.experimentId)}'
									});"
									   onmouseover="delayedTask.delay(2000, showDialog, this, ['TrialDetail_${anadivid}', {
										   title: '${ar.experimentAccession}',
										   url: '${createLink(action:'expDetail',id:ar.experimentId)}'
									   }]);"
									   onmouseout="delayedTask.cancel();">
										<img alt="Study Dtl" src="${resource(dir: 'images', file: 'view_detailed.png')}"
											...
											style="font-weight: bold; color: #339933;">${ar.experimentAccession}</span></a>
									-
									</g:if>

								<!-- analysis display -->
									<a onclick="showDialog('AnalysisDetail_${anadivid}', {
										title: '${analysis.shortDescription.replaceAll("'","\\\\'")}',
									   ...
									   id="AnalysisDetail_${anadivid}_anchor">
										<img alt="Analysis" src="${resource(dir: 'images', file: 'analysis.png')}"
										     style="vertical-align: top;"/> ${content}</a>
								</div>
							</td>

							<!--  content links -->
							<td width="10%" align="right">&nbsp; <g:link class="normal" action="downloadanalysisexcel"
							                                             id="${analysis.id}">
								<img alt="download analysis"
								     src="${resource(dir: 'images', file: 'Excel-16.gif')}"/>Excel</g:link>
							</td>

							<td width="10%" align="right">&nbsp;</td>
							<td width="10%" align="right">&nbsp;
							<g:if test="${analysis.files != null && !analysis.files.isEmpty()}">
								<g:createFileLink content="${analysis.files.iterator().next().content}"
								                  displayLabel="Analysis File"/>
							</g:if>
							</td>
							<!--  end of content -->
						</tr>

						<g:if test="${ar.assayAnalysisValueList}">
						<tr>
							<td colspan="3" width="100%">
								<a onclick="toggleDetail('${bmDivId}')">
									...
									<g:else>
										<g:if test="${ar.showTop()}">(top 5 of ${ar.bioMarkerCount}):</g:if>
							...
							<!-- don't display in TEA view -->
								<g:if test="${ar.teaScore == null}">
									<br>
									<g:each in="${ar.analysisValueSubList}" status="ai" var="analysisvalue">
									<g:if test="${ai > 0}">,</g:if>
									<g:createFilterDetailsLink id="${analysisvalue.bioMarkerId}"
									                           label="${analysisvalue.bioMarker?.name}" type="gene"/>
								...
								<g:set var="genes" value="${ar.geneNames}"/>
							</td>
						</tr>
						</g:if>
					</table>
