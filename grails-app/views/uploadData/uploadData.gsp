...
<g:uploadForm name='dataUpload' action='upload'>
	<div id="formPage1" class="formPage">
		...
		<div class="dataFormTitle" id="dataFormTitle1">
			<g:if test="${uploadDataInstance?.id}">Edit Metadata</g:if>
			<g:else>Upload GWAS results</g:else>
		</div>
		<div style="position: relative; text-align:right;">
			<a class="button"
			   href="mailto:${grailsApplication.config.com.recomdata.dataUpload.adminEmail}">Email administrator</a>
		...
		<table class="uploadTable">
			...
			<tr>
				...
				<td>
					...
					<tmpl:extSearchField width="600" fieldName="study" searchAction="extSearch" searchController="experiment"
					                     value="${study?.accession}" label="${study?.title}"
					                     paramString="'studyType':UPLOAD_STUDY_TYPE"/>
					<a id="studyChangeButton" class="upload" onclick="$j('#studyDiv').empty().slideUp('slow');
					changeField('study-combobox', 'study')">Change</a>
					<a style="margin-left: 32px;" id="studyBrowseButton" class="upload"
					   onclick="generateBrowseWindow();">Browse</a>
					<br/><br/>
			...
			<table class="uploadTable">
				<tr>
					...
					<td width="90%">
						...
						<g:select name="dataType" noSelection="${['null': 'Select...']}"
						          from="${['GWAS': 'GWAS', 'Metabolic GWAS': 'GWAS Metabolomics', 'EQTL': 'eQTL']}"
						          optionKey="${{ it.key }}" optionValue="${{ it.value }}"
						          value="${uploadDataInstance?.dataType}"/>
				...
				<tr>
					...
					<td colspan="3">
						<g:textArea name="description"
						            style="width: 90%; height: 100px">${uploadDataInstance.description}</g:textArea>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="buttonbar">
			...
			<g:actionSubmit class="upload" value="Upload File" action="uploadFile" id="uploadFileButton"
			                onclick="validateFile(event)"/>
	...
	<div id="formPage2" class="formPage" style="display: none;">
		...
		<div style="position: relative; text-align:right;">
			...
			If you are unable to locate the relevant autocomplete fields, email the administrator by clicking the button above.
			</div>
		</div>
	...
	<%-- Appalling hack from previous version - leaving this in until I can sort it properly --%>
		...
		<table class="uploadTable">
			...
			<tr class="borderbottom bordertop">
				<td id="tagsLabel"><font color="Red">*</font>Phenotype:</td>
				<td colspan="3">
			...
			<tr>
				<td>Population:</td>
				<td><g:textField name="population" value="${uploadDataInstance.population}"/></td>
				<td>Sample Size:</td>
				<td><g:textField name="sampleSize" value="${uploadDataInstance.sampleSize}"/></td>
			</tr>
			<tr>
				<td>Tissue:</td>
				<td><g:textField name="tissue" value="${uploadDataInstance.tissue}"/></td>
				<td>Cell Type:</td>
				<td><g:textField name="cellType" value="${uploadDataInstance.cellType}"/></td>
			</tr>
				...
				<td colspan="3">
					...
					<div style="background-color: #E4E4E4; float:left; padding: 8px; border-radius: 8px;">
						...
						<div style="float: left; margin-right: 8px">
							...
							<g:select style="width: 400px" name="genotypePlatformVendor" noSelection="${['null': 'Select...']}"
							          from="${snpVendors}" onChange="loadPlatformTypes('genotypePlatform', 'SNP')"/>
						</div>
						<div style="float: left">
							<div class="textsmaller">Platform</div>
							<g:select style="width: 200px" name="genotypePlatformName" from="${snpVendors}"
							          onchange="addPlatform('genotypePlatform')"/>
			...
			<tr>
				...
				<td colspan="3">
					<g:select name="genomeVersion" noSelection="${['null': 'Select...']}"
					          from="${['HG18': 'HG18', 'HG19': 'HG19']}" optionKey="${{ it.key }}"
					          optionValue="${{ it.value }}" value="${uploadDataInstance.genomeVersion}"/>
				</td>
			</tr>
			<tr id="expressionPlatformRow" class="bordertop borderbottom">
				<td>Expression Platform:</td>
				<td colspan="3">
					...
					<div style="background-color: #E4E4E4; float:left; padding: 8px; border-radius: 8px;">
						...
						<div style="float: left; margin-right: 8px">
							<div class="textsmaller">Vendor</div>
							<g:select style="width: 400px" name="expressionPlatformVendor"
							          noSelection="${['null': 'Select...']}" from="${expVendors}"
							          onChange="loadPlatformTypes('expressionPlatform', 'Gene Expression')"/>
						</div>
						<div style="float: left">
							<div class="textsmaller">Platform</div>
							<g:select style="width: 200px" name="expressionPlatformName"
							          onchange="addPlatform('expressionPlatform')" from="${expVendors}"/>
							<select id="expressionPlatform" name="expressionPlatform" multiple="multiple"
							        style="display: none;">
								<g:each in="${expressionPlatforms}" var="value">
									<option selected="selected" value="${value.key}">${value.value}</option>
								</g:each>
			...
			<tr>
				<td>Model Name:</td>
				<td><g:textField name="modelName" value="${uploadDataInstance.modelName}"/></td>
				<td>Model Description:</td>
				<td><g:textField name="modelDescription" value="${uploadDataInstance.modelDescription}"/></td>
			</tr>
			<tr class="borderbottom">
				<td>Statistical Test:</td>
				<td><g:textField name="statisticalTest" value="${uploadDataInstance.statisticalTest}"/></td>
				<td>P-value cutoff &lt;=</td>
				<td><g:textField name="pValueCutoff" value="${uploadDataInstance.pValueCutoff}"/></td>
			</tr>
			<tr class="borderbottom">
				<td>Research Unit:</td>
				<td>
					...
					<g:select style="width: 300px" name="researchUnitName" noSelection="${['null': 'Select...']}"
					          from="${ResearchUnits}" onchange="addResearchUnit('researchUnit')"/>
				...
				<td style="width:190px;">
					<input type="checkbox" style="width:20px;" name="sensitiveFlag"
					       value="${uploadDataInstance.sensitiveFlag}" id="sensitiveFlag"
					       onclick="isDataSensitive();">Is Data Sensitive?
				</input>
				</td>
				<td><g:textField name="sensitiveDesc" value="${uploadDataInstance.sensitiveDesc}"/></td>
			</tr>
		</table>
		<div class="buttonbar">
